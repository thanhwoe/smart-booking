...
export class ClerkWebhookService {
  ...
  async handleWebhook(
    ...
    try {
      ...
      this.trackService.capture({
        event: 'clerk_webhook_signature_verification_failed',
        distinctId: 'system',
        properties: {
          error: err as Error,
        },
  ...
  private async handleUserCreated(data: ClerkUserData) {
    ...
    await this.userRepository.syncClerkUser({
      clerkId: data.id,
      email,
      name,
      phone,
  ...
  private async handleUserDeleted(data: ClerkUserData) {
    await this.userRepository.deleteByClerkId(data.id);
